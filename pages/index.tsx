...
key={i}
d={s.d}
fill={i === active ? "url(#slice)" : "rgba(255,255,255,0.88)"}
...
/* Full background: soft gradients + subtle butterfly pattern */
.page{
min-height:100vh;
background:
...
.logo{ display:flex; align-items:center; gap:10px; font:700 16px Inter, system-ui, sans-serif; color:var(--ink); }
.logo span{
...
.sub{
margin:0; font:700 clamp(20px, 2.6vw, 28px) Inter, system-ui, sans-serif; color:var(--ink);
}
.tag{
margin:10px 0 24px; font:400 clamp(16px, 2vw, 18px) Inter, system-ui, sans-serif; color: var(--ink-dim);
}
...
.wheel{ width: min(520px, 100%); height:auto; display:block; }
.slice{ transition: filter .15s ease, transform .05s ease; cursor:pointer; }
.slice:hover{ filter: brightness(1.02) drop-shadow(0 2px 6px rgba(17,24,39,.12)); }
